...
function dxray_get_node_type() {
  ...
  foreach ($aNtypes as $oType) {
    ...
    $aMethods = array();
    $ID = $GML->addNode($oType->name, $aAttributes, $aMethods);
  }

  $file = DXRAY_OUTPATH . '/uml-' . date('d-m-Y_H-i-s') . '.graphml';
...
function dxray_get_fields_info() {
  ...
  foreach ($fields as $field) {
    ...
    $ID = $GML->addNode($field['field_name'], $aAttributes, $aMethods);
...
function _dxray_parse_bundles(&$IDFIELD, &$bundles, &$REID, &$GML) {
  foreach ($bundles as $entity => $items) {
    ...
    if (!isset($REID['nodes'][$entity])) {
      ...
      $ID = $GML->addNode($entity, array(), array(), $options);
    ...
    foreach ($items as $name) {
      if (!isset($REID['nodes'][$name])) {
        ...
        $ID = $GML->addNode($name, array(), array(), $options);
        $REID['nodes'][$name] = $ID;
      }
